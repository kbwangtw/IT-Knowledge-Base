...
可以把叢集想成三間仍能互相通話的倉庫。管理系統知道倉庫還在，但實際讀寫資料的服務拿著錯誤通行證，所以進不去。這就是「quorum 正常，儲存卻不能用」的差別。

本次 MON 裡有可用金鑰，出問題的是服務或掛載端使用的副本。修復是核對後同步有效內容，不是再產生一套新金鑰。
...
這次的診斷順序是：先確認叢集與日誌，再比對實際使用的金鑰，逐個恢復後逐層驗收。看到容量 0、掛載失敗或備份卡住時，先把故障位置查清楚，才能選對修復動作。
